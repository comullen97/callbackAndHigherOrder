...
const faveColorFinder = color => {
  if(color === "red") {
    ...
    return "you need to rethink your fav color"
  }
}
...
function emailCheck(email) {
  ...
  if (email.includes('@')) {
    ...
    return 'must provied a valid email'
  }
}
...
function checkArrayAscending(arr) {
  let comparisonValues = arr[0]
  for(let i = 1; i < arr.length - 1; i++) {
    if (arr[i] <= comparisonValues) {
      return false
    } else {
      comparisonValues = arr[i]
    }
  }
  return true
}
